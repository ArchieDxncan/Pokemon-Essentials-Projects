...
allowIneligible = false
  pbFadeOutIn {
    ...
    chosen = screen.pbChooseForm(allowIneligible)
    }
    if chosen[2] >= 0
...
def pbChooseForm(allowIneligible = false)
  @party = $player.party
  annot = []
  choice = []
  pkmnid = -1
  ...
  formcmds_hash = {}
	...
	elig = false if pkmn.egg? || pkmn.shadowPokemon? || pkmn.cannot_trade || 
	  ...
	  MultipleForms.hasFunction?(pkmn, "getUltraBurstForm") ||
	  MultipleForms.hasFunction?(pkmn, "getFormOnLeavingBattle") ||
	  species_data.has_flag?("SpecialForm")
   ...
   loop do
    ...
    # MODIFIED: Don't reset moves or change ownership info
    # We're keeping the original trainer, name, and obtain method
    ...
    # Keep the original record_first_moves call
    yourPokemon.record_first_moves
    
    # Trading Charm effects still apply if applicable
      ...
      loop do
        ...
        if selected.nil?
          ...
          if MultipleForms.hasFunction?(pokemon, "getForm")
            ...
            pbMessage("Sorry. This Pokemon is too special for me to do anything with.")
            next
          end
          formcmds = []
          form_name = [] 
